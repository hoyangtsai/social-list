...
function renderUserList(data) {
  let htmlContent = ''
  data.forEach((user) => {
    const heartClass = bestFriends.some(f => f.id === user.id) ? 'fas fa-heart' : 'far fa-heart';
    htmlContent += `
...
function addToFavorite(id) {
  ...
  const addHeart = document.querySelector(`button[data-id="${id}"] > i.fa-heart`);
...
axios
  .get(INDEX_URL)
  .then(async (response) => {
    bestFriends = await JSON.parse(localStorage.getItem('bestFriends') || '[]');
    await users.push(...response.data.results)
    await renderPaginator(users.length)
    await renderUserList(getUsersByPage(1))
  })
